...
22. `deac427` — lease удерживается до подтверждённой доставки результата.
23. `bf6c1f5`–`3897323` — атомарный settlement job/status/balance/counter и изолированный smoke-test.
24. `f4f97db` — worker переведён со старых раздельных charge/refund/status на settlement endpoint.
25. `8a4e681` — единый потокобезопасный PocketBase client для обоих Go-сервисов.
26. `0f257e5` — единый контракт общей конфигурации в стиле `main`.
27. `eca5308` — единый streaming multipart для PocketBase, FaceSwap и Telegram.
28. `6ac8a02` — persisted face-session хранит identity Telegram update и не превращает повторную доставку первого фото в target после restart.
29. `cc439d4` — общий startup-cleaner удаляет остатки из cache-dir Telegram-бота и job-manager до начала работы.
30. `0c23f42` — FaceSwap полностью очищает media sessions при старте вместо age-based уборки только старше суток.
...
- общий модуль `shared` отдельно проходит `go test -race ./...` и `go vet ./...`; старые копии token mutex/refresh/request и три реализации streaming multipart удалены из сервисов;
- образы `job-manager` и `telegram-bot` с локальным модулем `shared` собраны на `gpu-box` из ограниченного root build-context; runtime job-manager сообщает SHA `eca53085`;
- атомарный settlement smoke на `eca5308` подтверждает повторяемые charge/fail/complete без двойного списания, возврата или счётчика;
- реальный двухфайловый `POST /swap-photo` после multipart-рефакторинга вернул `200` и валидный JPEG `512×512`, 97951 байт.
- Telegram E2E на runtime `6ac8a02`: poller авторизовался как `@BoT_Sosiska_BoT`, забрал накопленные `/start` без `409 Conflict` и продолжил long polling;
- два одинаковых JPEG, отправленных отдельными Telegram update, корректно интерпретированы как source и target; создана ровно одна задача `nyl1n56czztwpgr` с `request_key=telegram:838237611`;
- задача прошла `queued → processing → sending → completed`, GPU endpoint `/swap-photo` вернул `200`, а итоговое изображение доставлено в Telegram;
- PocketBase после E2E показывает `attempts=1`, `price=1`, баланс `200→199`, `face_replace_count=0→1`; persisted face-session очищена полностью.
- startup-cleanup проверен реальным рестартом тех же контейнеров на `gpu-box`: в cache-dir bot, worker и FaceSwap заранее созданы контрольный файл и вложенный каталог; после рестарта все три каталога содержат `0` записей;
- runtime-логи `0c23f42` подтверждают удаление двух остатков каждым сервисом до авторизации/приёма задач; FaceSwap после очистки снова перешёл в `healthy`, Telegram poller продолжил `getUpdates` без конфликта.
...
После остановки другого consumer пользователем opt-in profile `telegram-e2e` запущен в изолированном стеке `/opt/faceswaper-codex-test`. Token подтверждён runtime-авторизацией как `@BoT_Sosiska_BoT`; Telegram Web открыт в том же чате (`PrivateAI`, id `5958345794`).

Poller забрал три ранее накопленных `/start`, ответил на них и продолжил long polling без конфликта. Затем через Telegram Web отправлены два отдельных update с одним тестовым JPEG. Первый сохранил face-session и показал подсказку, второй создал face job. Воркер обработал её на RTX 5080 за 57 секунд первого inference, отправил результат пользователю и атомарно завершил задачу со списанием одной монеты. Тестовый poller оставлен активным для последующих проверок; production-каталоги и чужие контейнеры не затронуты.
