...
Безопасный read-only preflight через локальный Telegram Bot API подтвердил: token валиден для `@BoT_Sosiska_BoT`, webhook отсутствует, `pending_update_count=0`. Открытый Telegram Web при этом находится в чате `@NoKnAbThBo_Bot`, то есть не с тем ботом, чей token лежит в `.env`. Telegram image собран, но контейнер poller не создавался; embedded SHA проверен offline с `--network none`.

Для безопасного последнего E2E нужен либо token от открытого `@NoKnAbThBo_Bot`, либо переход в Web к `@BoT_Sosiska_BoT` и явное разрешение на короткий polling cutover с текущим токеном. После этого тест состоит из запуска profile `telegram-e2e`, команды в Telegram Web, прохождения `queued → processing → sending → completed` и проверки полученного изображения.
