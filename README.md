...
- (опционально) `FACE_SWAP_SECRET`: если задан, требуется заголовок `X-API-KEY`; по умолчанию пусто (как раньше)
- (опционально) `FACE_SWAP_PUBLIC_URL`: базовый URL для выдачи `download_url` (по умолчанию пусто — локальные пути)
- (опционально) `MAX_IMAGE_BYTES`: лимит загрузки изображений в байтах; `0` = без лимита (поведение как раньше)
- (опционально) `MAX_VIDEO_BYTES`: лимит загрузки видео в байтах; `0` = без лимита (поведение как раньше)
- (опционально) `RATE_LIMIT_MAX` и `RATE_LIMIT_WINDOW`: лимит запросов по IP; `RATE_LIMIT_MAX=0` отключает (по умолчанию выключено)
